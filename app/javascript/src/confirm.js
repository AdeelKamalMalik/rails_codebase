...
function insertConfirmModal(message, element, button) {
  let buttonText = element.dataset.turboConfirmButtonText || 'Confirm'
  let confirmInput = ""
  ...
  let content = `
    <div id="${id}" class="z-50 bg-black/10 animated fadeIn fixed top-0 left-0 w-full h-full table bg-opacity-75">
      <div class="table-cell align-middle">
        ...
        <div class="bg-white mx-auto rounded shadow p-8 max-w-md dark:bg-gray-800">
          ...
          <div class="flex justify-end items-center flex-wrap gap-2 mt-6">
            <button data-behavior="cancel" class="btn btn-white border-gray-400">Cancel</button>
            <button data-behavior="commit" class="btn btn-primary focus:outline-none">${buttonText}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  if (confirmText) {
    ...
    modal.querySelector("input[data-behavior='confirm-text']").addEventListener("input", (event) => {
      ...
      if(event.target.value == confirmText) {
        commitButton.classList.add("btn-danger")
        commitButton.classList.remove('text-gray-500', 'bg-gray-100')
      } else {
        commitButton.classList.remove("btn-danger")
        commitButton.classList.add('text-gray-500', 'bg-gray-100')
      }
    })
  }
